...
function SessionPill({
  ...
  return (
    <button
      ...
      className="flex w-full items-center gap-1.5 overflow-hidden rounded-sm border border-hairline bg-coal-2 px-1.5 py-1 text-left outline-none transition-colors hover:border-hairline-2 hover:bg-coal-3 focus-visible:ring-2 focus-visible:ring-gold/30"
    >
      <span
        aria-hidden
      ...
      />
      <span className="truncate font-mono text-[0.5625rem] text-ash-dim">
...
export function MonthGrid({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-hairline bg-coal">
      {/* Weekday header */}
      <div className="grid grid-cols-7 border-b border-hairline bg-ink-2">
        {WEEKDAY_LABELS.map((d) => (
          <div
            key={d}
            className="px-2 py-2 text-center font-mono text-[0.625rem] uppercase tracking-wide text-ash-dim"
          >
            {d}
          </div>
        ))}
        ...
        {days.map((dayTs) => {
          ...
          return (
            ...
            >
              <span
                className={cn(
                  "grid size-6 shrink-0 place-items-center rounded-sm font-mono text-[0.6875rem]",
                  isToday && "bg-gold font-bold text-gold-ink",
                  !isToday && inMonth && "text-bone",
                  !isToday && !inMonth && "text-ash-dim",
                )}
              >
              ...
              <div className="flex flex-col gap-1">
                ...
                {overflow > 0 && (
                  <span className="px-1 font-mono text-[0.5625rem] text-ash-dim">
                    +{overflow} more
                  </span>
                )}
              </div>
