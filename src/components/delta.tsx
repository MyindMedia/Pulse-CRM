	...
	return (
		<DeltaContext.Provider value={{ value }}>
			{variant === "badge" ? (
				<Badge
					className={cn(
						"gap-1 border-none normal-case tabular-nums [&_svg]:size-3.5 [&_svg]:shrink-0",
						className
					)}
					data-slot="delta"
					tone={value > 0 ? "positive" : value < 0 ? "critical" : "neutral"}
					{...(props as React.ComponentProps<typeof Badge>)}
				/>
			) : (
				<div
					className={cn(
						"inline-flex items-center gap-1 text-steel tabular-nums",
