  ...
  return (
    <motion.div variants={fadeUp}>
      <Link
        ...
        className="group block h-full overflow-hidden rounded-lg border border-hairline bg-coal transition-all duration-150 hover:border-hairline-2 hover:bg-coal-2"
      >
        ...
        <div className="space-y-3 p-5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="truncate font-display text-base font-semibold tracking-tight text-bone group-hover:text-gold-bright">
                {room.name}
              </h3>
              {room.roomType && (
                <p className="mt-0.5 text-xs capitalize text-ash">{room.roomType}</p>
              )}
            </div>
            <ArrowUpRight className="size-4 shrink-0 text-ash-dim transition-colors group-hover:text-gold" />
          </div>
