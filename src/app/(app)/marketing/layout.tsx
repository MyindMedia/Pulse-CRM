 ...
 *
 *  Wrapped in CapabilityGuard the same way payments/reports/settings are
 *  (matches the nav entry's `capability: "marketing.read"`): every current
 *  studio role holds marketing.read, but an agency viewer deep-linking in
 *  without it would otherwise hit the backend's throw on first render and
 *  land on ShellErrorBoundary's generic screen instead of a friendly denial. */
export default function MarketingLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  return (
    <CapabilityGuard cap="marketing.read">
