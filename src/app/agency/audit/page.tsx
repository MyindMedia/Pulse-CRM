      ...
      <div className="overflow-x-auto rounded-lg border border-graphite/50 bg-coal/40">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-hairline">
            {(rows ?? []).map((r) => (
              <tr key={r._id}>
                <td className="p-3 text-xs text-steel/70">
                  {new Date(r._creationTime).toLocaleString()}
                </td>
                <td className="p-3 text-xs text-steel">{r.viewerId}</td>
                <td className="p-3 font-meta text-xs text-bone">{r.action}</td>
                <td className="p-3 text-xs text-steel/70">{r.orgId ?? "-"}</td>
                <td className="p-3 text-xs">
                  <span className={r.result === "allow" ? "text-positive" : "text-critical"}>
                    {r.result}
                  </span>
                </td>
                <td className="p-3 text-xs text-steel/70">{r.reason ?? ""}</td>
              </tr>
            ))}
            {rows && rows.length === 0 && (
              <tr>
                <td colSpan={6} className="p-6 text-center text-sm text-steel/70">
                  No audit events yet.
                </td>
              </tr>
            )}
