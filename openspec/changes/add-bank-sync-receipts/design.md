...
`receipts.attach` validates type/size using storage metadata, meters storage, inserts `reading`, and schedules `receipts.extract`. The internal action sniffs the document bytes and calls the receipt-only `lib/receiptAI.completeReceiptVisionJSON` adapter. `RECEIPT_AI_PROVIDER=gemini` selects the paid Gemini API with `GEMINI_RECEIPT_MODEL` (default `gemini-3.5-flash-lite`); `openai` or an unset provider retains the existing OpenAI helper, with optional `OPENAI_RECEIPT_MODEL`. Selection is explicit: errors never send the document to another provider. Gemini sends images/PDFs inline with JSON schema, injection guard, `store:false`, and a 90-second timeout; GIF stays available for manual review with an explanatory message. Scalar types are checked before the existing date, amount, currency, four-digit-card and confidence validation. Missing vendor/date/total or confidence below 0.6 produces `needs_review`. Successful reads charge `ai_credits`, record the model in the audit, and use existing deterministic reconciliation. Web and iOS share this backend path.
