...
### Addendum 2026-09-07 (evening) — round two on the yellow pieces, and punch alerts

- **Punch alerts (Lawrence, mid-round):** owners and managers get a device push for every clock-in and clock-out on the team (`convex/lib/punchAlert.ts`, wired in `timeclock.clockIn/clockOut`). Audience = studio owners + managers with a Clerk id, never the puncher, `strictAudience`. Words: "Sienna Cole clocked in · 9:52 AM · The Loft [· not on the rota]" / "clocked out · 6:04 PM · 7.8h on the clock". Deployed to prod.
- **Flows:** the tab bar is decided by role for order and first screen, then trimmed by the web's own capabilities (`MobileTab`, `PulseFlow.tabs(capabilities:)`, pinned in FlowTests against `src/lib/nav.ts`). Owners/managers reach Patch and Prep from the last two Dashboard tiles. Critic picked the phone in round 2.
- **Reminders:** the phone arms the server's moments (T-10, T+10 not clocked in, rostered end, OT-15, OT) from the next 48 h of its mirror; the studio stops pushing `you10`/`nc` to a phone that declares `localClock`, and the crew's `s10` alert is not sent to the person it names. Critic picked the phone's reminders over the server's pushes.
- **Non-goals still:** agency members' shared active studio; web `main`/Netlify untouched (feature branch only).

### Addendum 2026-09-10 — App Store readiness (goal: pass App Review)

Three parallel audits (review guidelines vs code, metadata/assets/URLs, Release build + archive) live in the session scratchpad `appstore/`. Decisions and assumptions: iPhone-only, portrait, category Business; no purchases in the app (3.1 clear); the app keeps Google sign-in, so **Sign in with Apple is required (4.8)** and is enabled on the app side (entitlement signed into the device build) — the Clerk instance must enable Apple with an Apple Services ID + Sign in with Apple key (Lawrence, Clerk dashboard + Apple portal). **Account deletion (5.1.1(v)) built**: `account:deleteMe` (login deleted at Clerk, seats anonymised to "Former teammate", devices removed, sole owner refused) + More → Account → Delete account. Privacy manifest added; policy and terms open from the app; `/support` page shipped and the privacy policy now names Apple push, Google/Apple sign-in and in-app deletion. **Review-only login** `appreview@studiopulse.tech` seated as manager on the demo studio `pulse-demo` (password in the scratchpad `appstore/review-account.txt`, not in git) — ⚠️ the instance does not sign in by password alone (`password.used_for_first_factor: false`, so it demands an email code after the password); Lawrence must enable password sign-in in the Clerk dashboard or the reviewer cannot get in. TestFlight upload needs either an App Store Connect API key (`tools/testflight.sh`, `ASC_KEY_ID`/`ASC_ISSUER_ID`) or the signed-in Xcode account with an app record created in App Store Connect first.

### Addendum 2026-09-10 (night) — money visibility, manager switch, staff dashboard, change log, versioning

Requests from Lawrence during the App Store pass, in his words where it matters:

- **Money is permission-bound everywhere.** "Engineers never see any money things; managers will and owners will." Applies to the web app, the iPhone app, and the device mirror. Built on the two capabilities already in the policy: `invoices.read` (client money: rates, deposits, balances, lifetime value, gear/software/licence/pipeline figures) and `insights.read` (the books: pay, payroll, expenses, revenue). The boundary is the server (`convex/lib/money.ts` + every read that returns those fields + the sync projection); views hide what the server already withholds.
- **Owner switch: "Managers can see money."** `orgs.managersSeeMoney` (default on), owner-only `orgs.setManagersSeeMoney`. Off, `lib/access.ts` withholds `invoices.read`, `invoices.send`, `finance.refund`, `insights.read` from every manager; managers keep bookings, schedule, team and the on-shift board (`timeEntries` mirror gate is now any-of `insights.read` / `schedule.manage`, rates stripped). A phone re-fetches when what its person may see changes (sync cursors carry a permission tag; `sync.cursorIsUsable` refuses a mismatched or untagged cursor, so every device re-snapshots once after this ships).
- **Money writes follow money reads.** Comp/discount, recording a deposit, texting a pay link, service prices, membership plans, licence sales need `invoices.send`; pay rates need `insights.read`; rooms need `rooms.edit`; a figure sent by someone without money (room rate, gear value, software cost, deal value, sync fee, cable cost) is ignored, and a floor booking is priced from the room's hourly rate.
- **iPhone, owners and managers with money:** full add / change / update parity with the web for financial records (bookings' money, invoices, payments, expenses/P&L, packages, payroll).
- **iPhone dashboard:** the studio's logo top-right of the header; a **non-financial dashboard** for engineers and staff, also with the logo.
- **Patch screen (iPhone):** Devices is the default view and comes first, Cables second.
- **Inventory edit for engineers:** a toggle when an owner or manager invites or adds staff (per-member `capabilityOverrides` `+equipment.edit`), changeable later.
- **Change log:** every change by anyone with edit rights — patch, inventory, check-ins, checklists, and the rest — recorded with who, what and when, readable by owners and managers from the Reports section.
- **Messages:** SMS all leaves from the shared Myind Sound GHL number and replies are matched by phone across every studio (first match wins, can land in the wrong studio). Options put to Lawrence: A in-app portal thread, B a number per studio, C route to the studio that last texted that phone. Recommendation C now, A for replying from the app. **Awaiting his pick.**
